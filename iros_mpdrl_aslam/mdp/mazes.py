...
def rect_maze_terrain(difficulty: float, cfg: "HfRectMazeTerrainCfg") -> np.ndarray:
    ...
    def _roughen_rect_edges(x0: int, y0: int, x1: int, y1: int):
        # Convert meters -> pixels
        bump_px = int(round(cfg.wall_edge_roughness_m / cfg.horizontal_scale))
        if bump_px <= 0 or cfg.wall_edge_bump_prob <= 0.0:
            return

        seg_len_px = max(1, int(round(cfg.wall_edge_bump_length_m / cfg.horizontal_scale)))

        # Per-wall deterministic RNG (so each wall segment gets a unique "signature")
        seed = (
            int(cfg.wall_edge_seed)
            ^ (x0 * 73856093)
            ^ (y0 * 19349663)
            ^ (x1 * 83492791)
            ^ (y1 * 2654435761)
        ...
        W, L = hf.shape  # (Wpx, Lpx)

        # Helper to stamp a bump rectangle in pixel coords
        def _stamp_px(ix0, iy0, ix1, iy1):
            ix0 = max(0, min(W, ix0))
            ix1 = max(0, min(W, ix1))
            iy0 = max(0, min(L, iy0))
            iy1 = max(0, min(L, iy1))
            if ix1 > ix0 and iy1 > iy0:
                hf[ix0:ix1, iy0:iy1] = H

        # LEFT/RIGHT edges: march along y
        for iy in range(y0, y1, seg_len_px):
            if rng.random() < cfg.wall_edge_bump_prob:
                # left outward bump
                _stamp_px(x0 - bump_px, iy, x0, min(y1, iy + seg_len_px))
            if rng.random() < cfg.wall_edge_bump_prob:
                # right outward bump
                _stamp_px(x1, iy, x1 + bump_px, min(y1, iy + seg_len_px))

        # BOTTOM/TOP edges: march along x
        for ix in range(x0, x1, seg_len_px):
            if rng.random() < cfg.wall_edge_bump_prob:
                # bottom outward bump
                _stamp_px(ix, y0 - bump_px, min(x1, ix + seg_len_px), y0)
            if rng.random() < cfg.wall_edge_bump_prob:
                # top outward bump
                _stamp_px(ix, y1, min(x1, ix + seg_len_px), y1 + bump_px)
    ...
    def stamp_rect(x: float, y: float, w: float, h: float):
        ...
        if x1 > x0 and y1 > y0:
            hf[x0:x1, y0:y1] = H
            _roughen_rect_edges(int(x0), int(y0), int(x1), int(y1))
...
class HfRectMazeTerrainCfg(HfTerrainBaseCfg):
    ...
    # Geometric "roughness" controls.
    wall_edge_roughness_m: float = 0.0        # 0 disables (meters, plan-view protrusion depth).
    wall_edge_bump_length_m: float = 0.10     # Meters along the edge.
    wall_edge_bump_prob: float = 0.25         # Probability per segment.
    wall_edge_seed: int = 13                  # Deterministic seed.
